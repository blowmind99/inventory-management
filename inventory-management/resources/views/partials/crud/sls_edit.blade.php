        ...
        <div class="content">
            <div class="block block-rounded">
                ...
                <div class="block-header block-header-default">
                    <h3 class="block-title">Edit Data Sales</h3>
                </div>
                <div class="block-content">
                    <form action="{{ url('/dashboard/sales/edit/'.$dataSales->id) }}" method="POST" enctype="multipart/form-data">
                        @csrf
                        <div class="row">
                            <div class="col-lg-12">
                                <input type="hidden" value="{{ $dataSales->id }}" name="id">
                                <div class="mb-4">
                                    <label class="form-label" for="example-text-input">Number</label>
                                    <input type="number" class="form-control" id="example-text-input" name="number" value="{{ $dataSales->number }}"required>
                                </div>
                                <div class="mb-4">
                                    <label class="form-label" for="datepicker">Date</label>
                                    <input type="text" class="form-control" id="datepicker" name="date" autocomplete="off" value="{{ \Carbon\Carbon::parse($dataSales->date)->format('m/d/Y') }}" required>
                                </div>
                                <div class="mb-4">
